...
inline bool canPlaceBlock(const State& base_state, int x, int y, int orientation) {
    const auto& block = ops::getBlock(base_state.current, static_cast<std::uint8_t>(orientation));
    return ops::canPlaceBlock(base_state.board, block, x, y);
}

inline bool moveHorizontally(SearchState& state, const State& base_state, int dx) {
    const int next_x = state.x + dx;
    if (!canPlaceBlock(base_state, next_x, state.y, state.orientation)) {
        return false;
    }
...
inline bool moveHorizontallyToWall(SearchState& state, const State& base_state, int dx) {
    const auto& block = ops::getBlock(base_state.current, static_cast<std::uint8_t>(state.orientation));
    int x = state.x;
    bool moved = false;
    while (ops::canPlaceBlock(base_state.board, block, x + dx, state.y)) {
        x += dx;
        moved = true;
    }
    if (!moved) {
...
inline bool moveDown(SearchState& state, const State& base_state) {
    ...
    if (!canPlaceBlock(base_state, state.x, next_y, state.orientation)) {
        return false;
    }
...
inline bool moveDownToFloor(SearchState& state, const State& base_state) {
    ...
    int y = state.y;
    bool moved = false;
    while (ops::canPlaceBlock(base_state.board, block, state.x, y + 1)) {
        ++y;
        moved = true;
    }
    if (!moved) {
...
inline bool rotate(SearchState& state, const State& base_state, Rotation rotation) {
    ...
    const auto& block = ops::getBlock(base_state.current, static_cast<std::uint8_t>(next_orientation));
    const auto& [kicks, length] = srs_table[static_cast<std::underlying_type_t<BlockType>>(base_state.current)][state.orientation][rotation_index];

    for (int i = 0; i < length; ++i) {
        const int next_x = state.x + kicks[i].x;
        const int next_y = state.y - kicks[i].y;
        if (ops::canPlaceBlock(base_state.board, block, next_x, next_y)) {
            state.x = static_cast<std::int8_t>(next_x);
            state.y = static_cast<std::int8_t>(next_y);
...
std::vector<PlacementSearchResult> findPlacements(const State& initial_state) {
    ...
    auto visited = std::make_unique<VisitedArray>();
    auto seen_placements = std::make_unique<PlacementArray>();
    std::vector<SearchState> queue;
    queue.reserve(MAX_STRUCTURAL_STATES);
    results.reserve(256);
    ...
    for (std::size_t head = 0; head < queue.size(); ++head) {
        ...
        { SearchState next = current; if (moveHorizontally(next, initial_state, -1)) { tryEnqueue(next, PlacementAction::LEFT); } }
        { SearchState next = current; if (moveHorizontally(next, initial_state, 1)) { tryEnqueue(next, PlacementAction::RIGHT); } }
        { SearchState next = current; if (moveHorizontallyToWall(next, initial_state, -1)) { tryEnqueue(next, PlacementAction::LEFT_WALL); } }
        { SearchState next = current; if (moveHorizontallyToWall(next, initial_state, 1)) { tryEnqueue(next, PlacementAction::RIGHT_WALL); } }
        { SearchState next = current; if (moveDown(next, initial_state)) { tryEnqueue(next, PlacementAction::SOFT_DROP); } }
        { SearchState next = current; if (moveDownToFloor(next, initial_state)) { tryEnqueue(next, PlacementAction::SOFT_DROP_FLOOR); } }
        { SearchState next = current; if (rotate(next, initial_state, Rotation::CW)) { tryEnqueue(next, PlacementAction::ROTATE_CW); } }
        { SearchState next = current; if (rotate(next, initial_state, Rotation::CCW)) { tryEnqueue(next, PlacementAction::ROTATE_CCW); } }
        { SearchState next = current; if (rotate(next, initial_state, Rotation::HALF)) { tryEnqueue(next, PlacementAction::ROTATE_180); } }
    }
